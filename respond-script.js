...
async function loadRant() {
    ...
    if (typeof API_CONFIG !== 'undefined' && API_CONFIG.USE_API) {
        ...
        questionCard.innerHTML = '<div style="display:flex;align-items:center;justify-content:center;padding:60px;"><span style="font-size:2rem;animation:spin 1s linear infinite;">⏳</span></div>';
    ...
    // Reset the question card HTML structure
    questionCard.innerHTML = `
        <h1 class="question-text" id="questionText"></h1>
        <p class="question-meta" id="responseTypesAllowed"></p>
    `;
    ...
    if (questionTextEl) questionTextEl.textContent = currentRant.question;
    ...
    const responseTypesEl = document.getElementById('responseTypesAllowed');
    if (responseTypesEl) responseTypesEl.textContent = `You can respond with: ${typesText}`;
...
submitDrawingBtn.addEventListener('click', () => {
    ...
    // Reduce size for faster upload (max 800px width)
    const maxWidth = 800;
    ...
    // Draw the transparent canvas on top (scaled)
    tempCtx.drawImage(canvas, 0, 0, tempCanvas.width, tempCanvas.height);
    
    // Export as JPEG with compression for faster upload
    const drawingData = tempCanvas.toDataURL('image/jpeg', 0.85);
    submitResponse('drawing', { 
            ...
            if (type === 'text') {
                ...
                response = await fetch(`${API_CONFIG.API_BASE_URL}/responses/drawing`, {
                    ...
                    body: JSON.stringify({
                        ...
                        brushColor: data.brushColor || '#000000',
