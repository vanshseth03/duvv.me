...
buyPremiumBtn.addEventListener('click', async () => {
    ...
    try {
        ...
        const confirmed = await showConfirm(
            'This will activate Premium features for testing. In production, this would process payment through Razorpay.',
            'Activate Premium? 💳',
            '💰'
        );
        ...
        // Step 2: Verify payment (simulated for testing)
        ...
        buyPremiumBtn.innerHTML = 'Success! 🎉';
        buyPremiumBtn.style.background = 'var(--lime-green)';
        
        setTimeout(async () => {
            closePremium();
            await showAlert(
                'Batman theme and Hints are now unlocked! Enjoy your premium experience. 🎉',
        ...
        await showAlert(
            'Failed to activate premium. Please try again or contact support.',
            'Purchase Failed ❌',
            '⚠️'
        );
        buyPremiumBtn.innerHTML = 'Upgrade Now via Razorpay';
...
function updatePremiumUI() {
    if (isPremium) {
        // premiumBtn.style.display = 'none'; // REMOVED: Keep button visible
        premiumBtn.classList.add('is-premium'); // Add styling class
        ...
        try {
            ...
            if (response.ok) {
                ...
                return data.duvvs.map(d => ({
                    ...
                    views: d.views || 0,
                    deactivated: !d.isActive
                }));
            }
            return [];
        } catch (error) {
            console.error('Error fetching duvvs from API:', error);
            return [];
        }
    }
    return [];
...
async function renderRants() {
    const rants = await getRants();
    await updateStats(); // Force update stats with fresh data
...
function addTestResponses() {
    ...
    if (firstRant.responses.length === 0) {
        ...
        saveRants(rants);
        console.log('✅ Test responses added to first rant');
    }
}
    ...
    if (typeof API_CONFIG !== 'undefined' && API_CONFIG.USE_API) {
        try {
            ...
            if (response.ok) {
                ...
                // Show success step
                const shareUrl = `${window.location.origin}/${username}/${newRant.id}`;
                document.getElementById('finalShareLink').value = shareUrl;
            ...
            console.error("Error creating duvv:", error);
            await showAlert(
...
async function openRantDetail(rantId) {
        ...
        try {
            ...
            if (response.ok) {
                ...
                rant.responses = data.responses.map(r => {
                    ...
                    // Construct full URL for audio responses
                    if (r.type === 'audio' && resp.audioUrl && !resp.audioUrl.startsWith('http')) {
                        resp.audioUrl = `${API_CONFIG.API_BASE_URL}${resp.audioUrl}`;
                    }
                    // Construct full URL for drawing responses
                    if (r.type === 'drawing' && resp.imageUrl && !resp.imageUrl.startsWith('http')) {
                        resp.imageUrl = `${API_CONFIG.API_BASE_URL}${resp.imageUrl}`;
                    }
                    return resp;
                });
            }
    ...
    if (rant.deactivated) {
        ...
        const shareUrl = `${window.location.origin}/${username}/${rantId}`;
...
function renderResponses(reset = false) {
    ...
    const html = chunk.map((response, index) => {
        ...
        if (response.type === 'text') {
            ...
            contentHtml = `<img src="${response.imageUrl}" alt="Drawing" class="response-drawing-preview" loading="lazy">`;
        }
...
function showDeleteConfirmation(responseIndex, cardElement) {
    ...
    dialog.innerHTML = `
        ...
        <div style="display: flex; gap: 12px;">
            <button id="cancelDeleteResponse" style="flex: 1; padding: 14px; border-radius: 12px; border: 2px solid var(--dark-border); background: transparent; color: var(--dark-text); cursor: pointer; font-weight: 600; font-size: 15px; transition: all 0.2s ease;">
                Cancel
            </button>
            <button id="confirmDeleteResponse" style="flex: 1; padding: 14px; border-radius: 12px; border: none; background: #ff4444; color: white; cursor: pointer; font-weight: 600; font-size: 15px; transition: all 0.2s ease;">
                Delete
            </button>
        </div>
    `;
...
function openResponseDetail(responseIndex) {
    ...
    if (currentResponse.type === 'text') {
        ...
        responseContent.innerHTML = `
            ...
            <img src="${currentResponse.imageUrl}" alt="Drawing response" class="detail-drawing">
        `;
    }
...
async function generateStoryImage(forceDownload = false) {
    ...
    if (navigator.share && navigator.canShare && navigator.canShare(shareData)) {
        ...
        console.log('Web Share API not supported or cannot share file');
        downloadFile(blob, `duvv-story-${Date.now()}.jpg`);
...
// Copy link via card button (needs async for getRants)
document.addEventListener('click', async (e) => {
    ...
    if (btn) {
        ...
        const rant = rants.find(r => r.id === rantId);
        if (rant && rant.deactivated) return; // Prevent copy for deactivated
        const link = `${window.location.origin}/${username}/${rantId}`;
...
async function generateQuestionCanvas() {
    try {
        if (!currentRant) {
            console.error('No current rant selected');
            return;
        }
    ...
    ctx.fillStyle = theme.text;
    ctx.globalAlpha = 0.6;
    ctx.fillText(`asked: give your reply by ${typesText}`, canvas.width / 2, textY);
    ctx.globalAlpha = 1.0;
...
// Copy Notification (Bottom Toast)
function showCopyNotification() {
    const notification = document.getElementById('copyNotification');
    
    if (!notification) return;
    
    // Update text to be clean
...
function initWelcomeDialog() {
    const welcomeOverlay = document.getElementById('welcomeDialogOverlay');
    ...
    function showStep(stepNum) {
        ...
        // Show current step
        const targetStep = document.querySelector(`.welcome-step[data-step="${stepNum}"]`);
        if (targetStep) {
        ...
        if (stepNum === totalSteps) {
            welcomeNext.textContent = 'Get Started';
        } else {
            welcomeNext.textContent = 'Next';
        }
    ...
    function closeWelcome() {
        ...
        setCookie('duvvWelcomeSeen', 'true', 365);
    }
    ...
    const welcomeSeen = getCookie('duvvWelcomeSeen');
    if (!welcomeSeen) {
        setTimeout(() => {
        ...
        if (rant) {
            ...
            if (typeof API_CONFIG !== 'undefined' && API_CONFIG.USE_API) {
                try {
                    ...
                    if (!response.ok) {
                        console.warn('Failed to update theme via API');
                    }
                } catch (error) {
...
async function deactivateRant(rantId) {
    ...
    if (typeof API_CONFIG !== 'undefined' && API_CONFIG.USE_API) {
        try {
            ...
            if (!response.ok) {
                console.warn('Failed to deactivate via API, using localStorage');
            }
        } catch (error) {
